---
title: OpenStack with MAAS and JuJu
slug: temp-slug-35
...
Everyone who tried OpenStack faced difficulty. Although the documentation is complet and pretty precise, you fastly realize that a full functional OpenStack architecture will be quite long to achieve. Regardless to architecture issues and new concepts you need to understand, one of the biggest source of problems is our human condition. You will forget a « ; » in your config file, you will mistype a password and you will spend too much time trying to figure out where you screwed up.

To solve this problem, some projects are born to help you in your OpenStack deployment, making it faster and safer. Furthermore, these projects will help you to focus on your architectures issues, which is more interesting than fill config files.

The two most leaded projects are from Canonical and SUSE. Canonical uses its homemade softwares MAAS and JuJu and SUSE uses its own suite : Suse Cloud. Those tools have the same philosophy, quickly turn any baremetal (MAAS in an acronym for Metal as a Service) server into a available node for OpenStack services.

The goal is to be able to scale and automatize the OpenStack services configuration.
